...
def parser(text):
    if text:
        normalise_text = text.replace("good bye", "good_bye").replace("show all", "show_all")
        return normalise_text.split()[0], normalise_text.split()[1:] # формуємо кортеж із назви функції і аргументів для неї
...
def add_func(args):
    ...
    if name in address_book.data.keys():
        for phone in phones:
            if int(phone) not in address_book[name].get_phones():
                address_book[name].add_phone(int(phone))
